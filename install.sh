...
echo "⚙️ Running Opiumware base installer..."
...
print_art_from_url() {
  local url="$1"
  if ! /usr/bin/curl -fsSL "$url" 2>/dev/null | while IFS= read -r line; do
    print_color "$ART_COLOR" "$line"
  done; then
    echo "⚠️  Failed to load art from $url"
  fi
}

print_art_from_url "https://pastebin.com/raw/8hBH9zht"
echo ""
print_art_from_url "https://pastebin.com/raw/ZNS2u3U6"
echo ""
print_art_from_url "https://pastebin.com/raw/CMBGrrqz"
